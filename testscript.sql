...
-- licenseID and piloting_experience
SELECT * FROM pilots;
SELECT licenseId, experience as piloting_experience FROM pilots;
...
SELECT * FROM pilots;
SELECT * FROM drones;
-- as num_locations (distinct hover locations)

SELECT pilots.username, pilots.licenseID, pilots.experience, COUNT(flown_by) as num_drones, COUNT(distinct hover) FROM pilots LEFT JOIN drones ON drones.flown_by=pilots.username GROUP BY pilots.username;
...
SELECT ingredients.iname as ingredient_name, drones.hover as location, payload.quantity as amount_available, min(payload.price) as low_price, max(payload.price) as high_price FROM ingredients 
LEFT JOIN payload ON payload.barcode=ingredients.barcode LEFT JOIN drones ON drones.tag=payload.tag;
