...
-- [29]
-- delivery_services.id, delivery_services.long_name, delivery_services.home_base, delivery_services.manager
SELECT delivery_services.id, delivery_services.long_name, delivery_services.home_base, delivery_services.manager FROM delivery_services 
LEFT JOIN drones ON drones.id=delivery_services.id 
LEFT JOIN payload ON drones.id=payload.id 
LEFT JOIN ingredients ON payload.barcode=ingredients.barcode GROUP BY payload.id;
...
-- cost_carried
SELECT sum(price * quantity) FROM payload JOIN ingredients ON payload.barcode=ingredients.barcode GROUP BY payload.id;
